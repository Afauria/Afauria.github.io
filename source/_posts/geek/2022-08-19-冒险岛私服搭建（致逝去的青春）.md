...
所以，在等树莓派之前就先折腾冒险岛私服吧！[冒险岛服务端的问题?](https://www.zhihu.com/question/39698446)
...
亲测了多个教程，这里先放总结。后面会简单介绍每个教程的操作，记录下过程，遇到问题可以作为参考。
...
2. 不带集成环境：需要自己装JDK和MySQL，并且改配置（适用于有一定基础的人，可以在Windows、Linux上运行）
...
<img src="2022-08-19-冒险岛私服搭建（致逝去的青春）/教程1资源.png" />
...
使用Java命令启动：`java -cp dist\* -Dnet.sf.odinms.wzpath=wz server.Start`
...
方法五：`sc delete mysql`卸载MySQL 5.7服务，再适用phpStudy中的MySQL服务
...
<img src="E:/GitPageBlog/AkiyamaBlog/source/_posts/geek/2022-08-19-%E5%86%92%E9%99%A9%E5%B2%9B%E7%A7%81%E6%9C%8D%E6%90%AD%E5%BB%BA%EF%BC%88%E8%87%B4%E9%80%9D%E5%8E%BB%E7%9A%84%E9%9D%92%E6%98%A5%EF%BC%89/%E5%8A%A0%E5%AF%86%E5%A5%97%E4%BB%B6%E5%BC%82%E5%B8%B8.png" />
...
<img src="E:/GitPageBlog/AkiyamaBlog/source/_posts/geek/2022-08-19-%E5%86%92%E9%99%A9%E5%B2%9B%E7%A7%81%E6%9C%8D%E6%90%AD%E5%BB%BA%EF%BC%88%E8%87%B4%E9%80%9D%E5%8E%BB%E7%9A%84%E9%9D%92%E6%98%A5%EF%BC%89/mSer%E8%BF%81%E7%A7%BB%E5%88%B0Linux.png" style="zoom:80%;" />
...
将sql文件中的`ROW_FORMAT = DYNAMIC `和`ROW_FORMAT = FIXED`批量删除掉即可。
...
>
> 反编译发现这个服务端使用的环境变量是`net.sf.odinms.wzpath`

# 服务端探索

高版本的端不太好找，都是付费的，还不一定能跑。
...
尝试破解一下，虽然没有成功，但还是学到了不少东西，以后有空再慢慢研究。
...
反编译查看jar：可以看到`org.bms.server.AuthCheckUtil`的main函数中输出了验证端url，并且拼接了淘宝订单号和`ProcessorId`

> wmic：Windows Management Instrumentation Command-line，Windows管理工具 

<img src="2022-08-19-冒险岛私服搭建（致逝去的青春）/176破解_AuthCheckUtil.png" style="zoom:100%;" />

运行AuthCheckUtil：`java -cp ;lib/*;jre8x64/lib/ext/log4j-core-2.14.0.jar;jre8x64/lib/ext/* org.bms.server.AuthCheckUtil`。输出http://47.99.96.61:8008/api/checkMapleAuthEn?ddhmid=`，即url

## IP地址转发

由于URL是IP地址，想着有没有类似host配置（将域名映射成IP）一样的东西，将目标IP映射到自己的服务器IP和端口上，然后模拟返回成功的结果。
...
通过curl命令可以成功访问本地的80端口结果`curl http://47.99.96.61:8008`

## 启动Web服务

将IP转发到自己的机器上后，需要开启Web服务，监听对应端口，模拟返回成功的结果

使用Apache自带Web服务：将网页放到这个目录下`/var/www/html/`，url带路径则新建文件夹

使用Java简易服务器：代码比较简单，修改端口号和路径之后，使用`javac -encoding "utf8" MyHttpServer.java`编译，再使用java命令运行class文件
...
* [冒险岛v83](https://forum.ragezone.com/f428/how-to-make-a-maplestory-741739/)：美国的客户端，无法安装。需要自行安装Java和MySQL环境。
...
* [2021冒险岛176至尊版](https://www.fengyewuyu.com/thread-4801-1-1.html)：176整合版，需要淘宝订单号
