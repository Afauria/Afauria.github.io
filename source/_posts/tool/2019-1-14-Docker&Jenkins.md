...
![Jenkins环境变量配置](/images/Jenkins/jenkins01.png)
...
![Jenkins环境变量配置](/images/Jenkins/jenkins02.png)
...
![Jenkins环境变量配置](/images/Jenkins/jenkins03.png)

### 1.4.2. Build配置

会在上面配置的工作区上执行脚本，有多种脚本可以执行，如gradle，shell等。可以写一些简单的脚本进行测试，比如打印java版本，查看当前目录文件等。gradle脚本选择刚才在全局工具配置中的名称

![Jenkins环境变量配置](/images/Jenkins/jenkins04.png)

点开左边的构建历史，查看某次构建，如下

![Jenkins环境变量配置](/images/Jenkins/jenkins05.png)
...
## 2.5. docker内的容器找不到主机路径，需要Volume挂载
...
## 2.6. Force GRADLE_USER_HOME to use workspace

这个选项是把gradle缓存之类的（就是平时用户目录下的.gradle/下的东西）都放到workspace里，网上说勾上，但这里我不建议勾选，不勾选他会下到`/root`目录下，所有项目共用（和平时一样）。

## 2.7. job左边的workspace里面有一个清扫工作空间，看到一篇文章说

![Jenkins成功图](/images/Jenkins/jenkins08.png)

**这里提醒一下，删除job不会删除项目，但是点击了清理工作空间，整个项目都会被删掉（慎用！！！！！）**

## 2.8. docker中使用vim，有的容器有（nexus），有的容器没有（nginx、jenkins）
...
## 2.10. 执行`compileDebug --stacktrace `出错，改为`gradlew compileDebugJavaWithJavac --stacktrace`
...
## 2.12. 共用缓存报错（Jenkins执行任务，本地使用IDE也在执行）
...
![Jenkins成功图](/images/Jenkins/jenkins14.png)