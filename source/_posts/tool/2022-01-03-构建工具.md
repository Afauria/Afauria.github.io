...
4. 链接（link）[链接器ld]：处理各个模块之间的引用和依赖，将目标文件链接到可执行文件或其他目标文件。生成可执行文件。
   1. 静态链接：目标文件直接进入可执行文件
      1. 编译静态库源码，生成.o目标文件：`gcc -c lib.c -o lib.o`
      2. 打包生成`.a`归档文件：`ar -q lib.a lib.o`
      3. 使用静态库编译，生成`.out`可执行文件：`gcc main.c lib.a -o main.out`
   2. 动态链接：可执行程序运行时加载目标文件
      1. 编译动态库源码，生成`.so`文件：`gcc -shared dlib.c -o dlib.so`
...
1. `gcc -c xxx.c`编译生成`.o`文件
2. 使用`ar`归档工具将所有`.o`文件打包生成`.a`文件：`ar -q libfoo xxx.o yyy.o `
3. 链接：`gcc -o main main.c -static -l -L. -lfoo`
...
1. `gcc -c xxx.c`编译生成`.o`文件
2. `gcc --shared -fPIC -o libfoo.so xxx.o yyy.o`编译生成`.so`文件
3. `gcc -o main main.c -lfoo `链接生成可执行程序（去掉lib和so前后缀）
4. 运行时从`/lib`、`/usr/lib`等路径下搜索

> `-fPIC`：使用地址无关代码（Position Independent Code）技术来生成文件。不加这个选项的话无法让其他程序共享。
>
> 原理：告诉GCC目标代码不要包含对具体内存位置的引用，运行时才确定具体的内存地址空间，由每个进程自行决定。
...
3. ldconfig指定的路径：`cat /etc/ld.so.cache`
4. `/lib`目录
5. `usr/lib`目录
