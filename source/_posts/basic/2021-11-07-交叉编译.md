...
Android和Java代码编译只需要编译成dex或class，不需要编译成目标代码，因此不需要交叉编译。但是Android中的Native开发还是需要使用交叉编译工具链，NDK中做了封装。
...
指定编译时的逻辑根目录，只在链接过程中起作用，作为交叉编译工具链搜索库文件的根路径。

> 编译过程中默认会在`/usr/include`、`/usr/lib`下查找引用的库和头文件，指定sysroot为dir之后会在`dir/usr/include`、`dir/usr/lib`下查找。

**sysroot本质是目标平台的依赖库，交叉编译时需要链接对应架构的平台库，通过sysroot在主机上找到目标平台的库和头文件。**

关于程序链接可以参考[构建工具](/2022/01/03/tool-2022-01-03-GNU和编译工具介绍/)

## 制作sysroot

sysroot本质是目标平台的依赖库，只要想办法下载到编译需要的依赖库即可。
...
以Docker+QEMU为例，创建arm64 sysroot

1. 在Linux主机上安装交叉构建工具：`apt install gcc-aarch64-linux-gnu g++-aarch64-linux-gnu libstdc++-8-dev-arm64-cross`

2. 在Linux主机上安装Docker

3. 安装QEMU，用于运行arm架构容器：`apt install qemu-user-static`

4. 运行Docker官方`arm64v8/ubuntu`容器：`docker run -it --name ubuntu-arm64 arm64v8/ubuntu:18.04`

5. 在容器中安装依赖库：根据需要安装，这里以Flutter eLinux为例
...
6. 将arm64平台的依赖库拷贝到Linux主机：`docker cp ubuntu-arm64:/ ubuntu-arm64-sysroot`

7. 此时可以在Linux主机使用flutter构建命令`--target-sysroot=ubuntu-arm64-sysroot`构建arm64位应用

> 直接在主机用apt安装只会安装当前平台的库，所以需要利用对应架构容器的包管理工具（apt）下载目标架构的平台库。
>
...
* [Cross-building from x64 to arm64](https://github.com/sony/flutter-elinux/wiki/Building-flutter-apps#case-1-use-docker--qemu)
