...
```shell
{flutter_sdk}/bin/cache/dart-sdk/bin/dart {flutter_sdk}/bin/cache/artifacts/engine/darwin-x64/frontend_server.dart.snapshot 
...
查看`AotAndroidAssetBundle`目标：调用`copyAssets`生成Flutter静态资源文件（如图片、字体文件，配置文件等）并拷贝，如果是Debug模式，会拷贝快照文件到flutter_assets中，而不是打包成`libapp.so`。

* `kernel_blob.bin`是Dart编译前端生成的Kernel快照文件，即`app.dill`
...
3. 了解Flutter代码的编译，如何生成产物，然后合并打包

关于Dart前端编译（生成Dart Kernel文件，即`.dill`）和AOT编译（生成目标代码）可以参考：[Dart简介](/2022/01/03/flutter-2022-01-03-Dart简介/)
