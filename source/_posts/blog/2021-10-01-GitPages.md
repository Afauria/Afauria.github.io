...
GitHub Pages本质就是一个GitHub仓库，只不过GitHub自动将仓库路径下的文件部署到了静态站点服务器`github.io`上，通过特定的url即可访问。

**注：GitHub Page可以有一个用户站点，以及多个项目站点**

* 用户站点路径为：`http(s)://<username>.github.io`
* 项目站点路径为：`http(s)://<username>.github.io/<repository>`
...
1. 【Settings-Pages】中选择分支和文件夹：`/`或者`/docs`
2. 本地生成静态文件
   1. 使用静态站点生成器生成静态博客站点，例如Hexo、Hugo
   2. GitHub内置了Jekyll支持，因此可以直接推送源工程，由GitHub远程构建
   3. 可以本地手写Html前端页面，入口为`index.html`
3. 使用Git Push命令将静态站点文件推到GitHub上。
...
* [GitHub Pages官方文档](https://docs.github.com/en/pages/getting-started-with-github-pages/about-github-pages)
