...
4. 广义上讲SPI是**一种让外部能够按照自己定制的规则实现功能的方式**，不单指Java语言中的接口

## 问题2

在SPI的情况下，插件需要依赖调用方，调用方也需要依赖插件具体实现。是否会产生依赖冲突呢，三者的依赖关系应该是怎么样的？

> 从编码上讲会存在依赖冲突，当然这个时候调用方不一定是直接依赖插件，也可以通过动态加载，如自定义ClassLoader、动态链接库等。
