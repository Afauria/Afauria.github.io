...
- 添加属性：配置禁止检查无用资源lint，该属性会继承到子元素：`tools:ignore="UnusedResources"`
...
public class AfauriaResIdDetector extends ResourceXmlDetector {

    private static final Implementation IMPLEMENTATION_RES_ONLY = new Implementation(
            AfauriaResIdDetector.class,
...
```java
public class AfauriaIssueRegistry extends IssueRegistry {
    @Override
    public List<Issue> getIssues() {
        return Arrays.asList(
                AfauriaResIdDetector.RES_ID_ISSUE
        );
    }
}
```
