---
layout: post
title: Android打包和反编译、应用安全
date: 2021-11-04
description: Android打包和反编译笔记，工具说明，应用安全checklist
categories: Android
tags: 
- Android
keywords: [android]
...
- [`dex2jar`](https://github.com/pxb1988/dex2jar)：将可运行文件classes.dex反编译为jar源码文件
  - `d2j-dex2jar classes.dex`得到`classes-dex2jar.jar`文件
...
```mermaid
graph LR
apk--apktool-->dex文件--dex2jar-->jar文件-->使用jd-gui查看java源代码
apk--apktool-->smali文件-->对照java源代码修改smali汇编代码-->打包-->重签名
apk--apktool-->资源文件-->修改资源文件-->打包
```

其他工具：

[jadx](https://github.com/skylot/jadx)：反编译dex、aar、aab、apk、资源等，包括可视化和命令行方式

[CrackTool](https://github.com/Jermic/Android-Crack-Tool)：反编译、签名、可视化工具

[jbe]()：字节码编辑器，用于查看和修改java类文件
...
5. class文件：Java字节码文件，在JVM中执行
4. dex文件：Dalvik字节码文件，在DVM中执行
5. smali文件：dalvik字节码文件，dex文件反汇编得到，相当于dalvik虚拟机的汇编语言，便于阅读

> javap：将class字节码文件反汇编，用于查看java汇编代码，了解JVM指令执行过程。JVM汇编指令参考[官方文档](https://docs.oracle.com/javase/specs/jvms/se7/html/jvms-6.html#jvms-6.5)
>
> 有助于了解原理，如
>
> * java枚举
...
1. odex文件(optimized dex)：针对系统和平台优化过的dex。ART上AOT编译过的机器码也保存在odex文件中。
   ...
   3. ART中Android O之后，将oat中原始dex保存到.vdex中，从vdex中提取出部分代码进行AOT编译和优化，生成odex。`odex+vdex=apk源码`。（AOT+JIT）
2. oat文件：本质是ELF格式文件。包含原始dex字节码(oatdata)和翻译过的本地代码(oatexec)
3. vdex文件：Android O新增，保存原始未压缩的dex。
