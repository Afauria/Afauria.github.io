...
原理：编译前端过程中javac进行词法分析和语法分析之后会生成**抽象语法树（AST）**，然后调用注解处理器（注解处理器相当于javac对外提供的插件），可以在这个阶段生成新的java文件。也可以**直接修改抽象语法树**
...
1. 需要自定义APT处理器
2. 需要手动拼接代理的代码：可以使用[JavaPoet](https://github.com/square/javapoet)，调用API生成java源代码。如果是Kotlin，可以使用[KotlinPoet](https://github.com/square/kotlinpoet)
4. 需要使用者手动调用代理类执行，或者通过门面对象，反射找到代理类并执行。如`ButterKnife.bind(this)`
4. 会生成大量代理类，导致类和方法数增多
5. 无法直接在源文件中插入或修改代码，而是生成代理类。
...
| 函数名  | 作用                                                |
| ------- | --------------------------------------------------- |
| accept  | 访问者模式，传入`TypeVisitor`。根据子类类型进行访问 |
...
以`AptActivity`为例，需要生成的类如下：APT的作用就是帮我们自动生成`XXX_ViewBinding`类，免去手写的过程
...
public class AptBinder {
    ...
    public static void bind(Activity activity) {
        ...
        //找到类名：由于library中无法拿到生成的类，因此需要使用反射来动态加载类
