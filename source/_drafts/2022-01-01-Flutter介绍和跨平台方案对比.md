...
缺点：Android、iOS分开维护，内容固定，动态化弱，更新需发版。
...
|                  | WebView                                          | Hybrid（WebView+）                                  | JS引擎+原生渲染                          | Dart+自绘渲染              |
| ---------------- | ------------------------------------------------ | --------------------------------------------------- | ---------------------------------------- | -------------------------- |
| 介绍             | 使用原生Web容器，JS引擎解析，WebKit或者Blink渲染 | 混合开发，WebView增强。通过JSBridge进行原生通信调用 | 类RN方案，JS引擎解析成原生组件，原生渲染 | Dart编译，提供渲染引擎自绘 |
| 虚拟机（运行时） | JS引擎（V8、JSC）                                | JS引擎+JSBridge                                     | JS引擎                                   | Dart VM                    |
| 渲染引擎         | WebKit、Blink                                    | WebKit、Blink                                       | Native渲染                               | Skia                       |
| 代表框架         | H5、PWA                                          | Cordova、Ionic、小程序、PhoneGap                    | RN（FaceBook）、Weex（阿里）、Hippy      | Flutter（Google）          |
| 性能             | 差                                               | 差                                                  | 原生                                     | 接近原生                   |
...
| 上手难度          | 较高               | 一般              | 一般                         |
| 框架程度          | 较重               | 较轻              | 重                           |
| 开发者            | FaceBook           | 阿里              | Google                       |
| 社区              | 丰富，Facebook维护 | 沉寂              | 刚刚出道小鲜肉，拥护者众多   |
| 支持              | Android、iOS       | Android、iOS、Web | Android、iOS、桌面、Fuchisia |
| 打包              |                    |                   |                              |
| 原生调用          |                    |                   |                              |
| 原生页面跳转      |                    |                   |                              |
| 热更新            | 支持               |                   | 不支持                       |
...
市面应用、社区、三方库，成熟度：容易过时，不对比了。目前应该是Flutter最火。

## 跨平台方案

### H5+原生混合开发

原理：页面运行在WebView浏览器内核中，通过JSBridge定义JS和原生API通信传输协议

代表框架：Cordova、Ionic、微信小程序、PhoneGap
...
原理：构建虚拟Dom树，通过JavaScriptCore

代表框架：ReactNative、Weex、快应用
...
1. 渲染时需要JS和原生通信
2. JS需要运行时编译（JIT）
3. 不同平台控件需要单独维护，复杂场景需要对原生控件进行扩展
...
![](2022-01-01-Flutter介绍和跨平台方案对比/Flutter预研过程.png)
